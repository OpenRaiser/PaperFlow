...
当前 Scholar 冷启动增强版会优先抓取这些信号：
- 主页兴趣标签
- 代表论文标题
- 论文作者行 / venue 行等轻量发表记录信息
- 分页后的更多发表记录
- 高频合作作者
- 引用统计概览

这些信号会统一转成一段 bootstrap text，再复用现有方向解析与画像合并逻辑；合作作者和机构信号还会额外写入 `author_heat` / `institution_heat`，而不是单独维护第二套冷启动规则。
