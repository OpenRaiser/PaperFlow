#!/usr/bin/env bash
# Run NL-Profile reranking baseline against clean baseline input.
set -euo pipefail

BENCHMARK_DIR="${1:-data/benchmark_full_24users_20260301_20260419_show20_with_reading}"

echo "============================================================"
echo "NL-Profile baseline with clean, contamination-safe input"
...
echo
echo "[2/2] Run NL-Profile reranking baseline..."
"${PYTHON_BIN}" experiments/main_experiment/run_baselines/run_nl_profile.py \
  --input-dir "${BENCHMARK_DIR}/baseline_clean_input" \
  --output-dir "${BENCHMARK_DIR}/main_experiment/nl_profile"

echo
echo "Done."
echo "Output: ${BENCHMARK_DIR}/main_experiment/nl_profile"
