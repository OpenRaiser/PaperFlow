#!/usr/bin/env bash
# Run SciNUP-Strict reranking baseline against clean baseline input.
set -euo pipefail

BENCHMARK_DIR="${1:-data/benchmark_full_24users_20260301_20260419_show20_with_reading}"

echo "============================================================"
echo "SciNUP-Strict baseline with clean, contamination-safe input"
...
echo
echo "[2/2] Run SciNUP-Strict reranking baseline..."
"${PYTHON_BIN}" experiments/main_experiment/run_baselines/run_scinup_strict.py \
  --input-dir "${BENCHMARK_DIR}/baseline_clean_input" \
  --output-dir "${BENCHMARK_DIR}/main_experiment/scinup_strict"

echo
echo "Done."
echo "Output: ${BENCHMARK_DIR}/main_experiment/scinup_strict"
