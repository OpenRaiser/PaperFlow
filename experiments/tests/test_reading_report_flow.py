...
def test_pdf_evidence_cleaner_filters_common_extraction_noise():
    noisy = (
        ...
        "This work was supported by grant 123.\n"
        "Figure 4. Visual comparison of novel view synthesis.\n"
        "We propose a two-stage planner that retrieves evidence before writing the report."
    )

    cleaned = reading_agent._clean_pdf_evidence_text(noisy)

    assert "Corresponding author" not in cleaned
    assert "Figure 4" not in cleaned
    assert "two-stage planner" in cleaned
    assert reading_agent._is_noisy_pdf_evidence_text("Corresponding author: jane@example.com") is True
    assert reading_agent._is_noisy_pdf_evidence_text(
        "We propose a two-stage planner that retrieves evidence before writing the report."
    ) is False


def test_recommendation_label_is_calibrated_by_relevance_evidence():
    assert (
        reading_agent.calibrate_recommendation_label(
            {"relevance_level": "must_read", "oracle_label": "strong_relevant", "oracle_score": 0.9},
            "推荐阅读",
            "pdf",
        )
        == "强烈推荐"
    ...
    assert (
        reading_agent.calibrate_recommendation_label(
            {"relevance_level": "high_relevant", "oracle_label": "relevant", "oracle_score": 0.52},
            "强烈推荐",
            "pdf",
        )
        == "推荐阅读"
    ...
    assert (
        reading_agent.calibrate_recommendation_label(
            {"relevance_level": "maybe_interested", "oracle_label": "weak_relevant", "oracle_score": 0.31},
            "强烈推荐",
            "pdf",
        )
        == "值得快速浏览"
    ...
    assert (
        reading_agent.calibrate_recommendation_label(
            {"relevance_level": "edge_relevant", "oracle_label": "irrelevant", "oracle_score": 0.1},
            "强烈推荐",
            "pdf",
        )
        == "按需阅读"
    ...
    assert (
        reading_agent.calibrate_recommendation_label(
            {"relevance_level": "high_relevant", "oracle_label": "relevant", "oracle_score": 0.55},
            "强烈推荐",
            "abstract",
        )
        == "推荐阅读"
