...
DEFAULT_PORT = 8080
DEFAULT_TUNNEL_NAME = "paperflow-webhook"
REQUIRED_ENV_VARS = (
    "FEISHU_APP_ID",
    "FEISHU_APP_SECRET",
...
def start_webhook_thread(port: int) -> tuple[threading.Thread, dict[str, str | None]]:
    webhook_server = importlib.import_module("deployments.feishu.webhook-server.scripts.webhook_server")
...
def start_ngrok_process(port: int, domain: str | None = None) -> tuple[subprocess.Popen[str], Any]:
    ...
    cmd = [ngrok_binary, "http"]
    if domain:
        cmd.append(f"--url={domain}")
    cmd.append(str(port))
...
def create_tunnel_via_api(port: int, tunnel_name: str, domain: str | None = None) -> dict[str, Any] | None:
...
def print_summary(port: int, public_url: str, ngrok_url_path: Path, feishu_url_path: Path) -> None:
    request_url = public_url.rstrip("/") + "/"
    scheduler = importlib.import_module("deployments.feishu.webhook-server.scripts.scheduler")
    print()
    print("=" * 60)
    print("PaperFlow Webhook + ngrok Ready")
    print("=" * 60)
...
def main() -> int:
    ...
    parser.add_argument(
        "--ngrok-domain",
        type=str,
        default=os.environ.get("NGROK_DOMAIN", "").strip() or None,
        help="Reserved ngrok static domain (e.g. paperflow-bot.ngrok-free.app). "
             "Defaults to NGROK_DOMAIN env var.",
    )
            ...
            if not wait_for_webhook(args.port):
                if startup_state.get("error"):
                    ...
                    print(
                        ...
                        "but /health did not respond as a PaperFlow webhook"
                    )
                else:
    ...
    if existing_tunnel:
        ...
        if ngrok_api_online():
            ...
            created_tunnel = create_tunnel_via_api(args.port, created_tunnel_name, domain=args.ngrok_domain)
            ...
            if args.ngrok_domain:
                print(f"[INFO] No local ngrok agent detected, starting ngrok with domain {args.ngrok_domain}...")
            else:
                print("[INFO] No local ngrok agent detected, starting ngrok...")
            started_ngrok_process, ngrok_log_file = start_ngrok_process(args.port, domain=args.ngrok_domain)
