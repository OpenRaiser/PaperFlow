...
# 添加项目根目录到路径（webhook_server.py 在 deployments/feishu/webhook-server/scripts/）
...
def get_async_task_lock_ttl_seconds() -> int:
    raw = os.environ.get("PAPERFLOW_ASYNC_TASK_LOCK_TTL_SECONDS", "1800").strip()
    try:
...
class FeishuEventHandler:
    ...
    def _find_role_by_chat_id(self, chat_id: str) -> Optional[str]:
        ...
        import json
        try:
            # 项目根目录是 PaperFlow 仓库根，data/roles.json 相对于项目根目录
            project_root = os.path.normpath(os.path.join(
                os.path.dirname(os.path.abspath(__file__)),
                "..", "..", "..", ".."  # deployments/feishu/webhook-server/scripts -> repo root
            ))
    ...
    def _route_to_coordinator(self, user_id: str, message: str, chat_id: str, sender_open_id: str = None) -> Dict[str, Any]:
        ...
        """
        try:
            _master_coordinator_module = importlib.import_module("agents.master-coordinator.main")
            MasterCoordinator = _master_coordinator_module.MasterCoordinator
    ...
    def _route_to_coordinator_async(
        ...
        thread = threading.Thread(
            target=target,
            name=f"paperflow-{user_id}-{int(time.time())}",
            daemon=True,
        )
        thread.start()
    ...
    def _route_pdf_reading_report_async(
        ...
        thread = threading.Thread(
            target=target,
            name=f"paperflow-pdf-{int(time.time())}",
            daemon=True,
        )
        thread.start()
...
class WebhookHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/health":
            ...
            try:
                scheduler_module = importlib.import_module("deployments.feishu.webhook-server.scripts.scheduler")
