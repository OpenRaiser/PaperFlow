...
LOGGER = logging.getLogger("paperflow-scheduler")
...
def scheduler_enabled() -> bool:
    return _env_flag("PAPERFLOW_SCHEDULER_ENABLED", True)


def get_scheduler_timezone_name() -> str:
    return os.environ.get("PAPERFLOW_TIMEZONE", "Asia/Shanghai").strip() or "Asia/Shanghai"
...
def get_daily_push_time() -> tuple[int, int]:
    return _parse_hhmm(os.environ.get("PAPERFLOW_DAILY_PUSH_TIME", "09:00"), "09:00")


def get_weekly_report_time() -> tuple[int, int]:
    return _parse_hhmm(os.environ.get("PAPERFLOW_WEEKLY_REPORT_TIME", "10:00"), "10:00")


def get_weekly_report_weekday() -> int:
    raw = os.environ.get("PAPERFLOW_WEEKLY_REPORT_WEEKDAY", "0").strip()
    try:
...
def get_poll_seconds() -> int:
    raw = os.environ.get("PAPERFLOW_SCHEDULER_POLL_SECONDS", "30").strip()
    try:
...
def get_scheduler_grace_minutes() -> int:
    raw = os.environ.get("PAPERFLOW_SCHEDULER_GRACE_MINUTES", "10").strip()
    try:
...
def get_max_retries() -> int:
    raw = os.environ.get("PAPERFLOW_SCHEDULER_MAX_RETRIES", "2").strip()
    try:
...
def get_retry_backoff_minutes() -> int:
    raw = os.environ.get("PAPERFLOW_SCHEDULER_RETRY_BACKOFF_MINUTES", "15").strip()
    try:
...
def get_daily_compensation_hours() -> int:
    raw = os.environ.get("PAPERFLOW_SCHEDULER_DAILY_COMPENSATION_HOURS", "6").strip()
    try:
...
def get_weekly_compensation_hours() -> int:
    raw = os.environ.get("PAPERFLOW_SCHEDULER_WEEKLY_COMPENSATION_HOURS", "6").strip()
    try:
...
def get_scheduler_alert_chat_id() -> str:
    return str(os.environ.get("PAPERFLOW_SCHEDULER_ALERT_CHAT_ID", "")).strip()
...
def start_scheduler_thread() -> Optional[threading.Thread]:
    ...
    if not scheduler_enabled():
        LOGGER.info("Scheduler disabled by PAPERFLOW_SCHEDULER_ENABLED")
        return None
